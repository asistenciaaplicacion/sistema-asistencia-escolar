const API =
'https://script.google.com/macros/s/AKfycbx-Klzoc4VQtN6FclfGRml4jmL53KXDQW_mCmLG6VFGsSl40o73mbEVd2TsDgjj4TIM/exec';
...
async function generarPDF(){
  ...
  await mostrarModalConfirmacion(
    ...
    const respuestaFolio =
      await fetch(
        API +
        '?accion=generarFolioTemporalPDF'
      );
...
new QRCode(
  ...
  setTimeout(() => {
    ...
    const alumno =
  contenedor.querySelector('h4')?.innerText || 'Alumno';

const grupo =
  contenedor.querySelector('.grupo')?.innerText || 'Sin grupo';
...
try{
  ...
  fetch(API, {
    ...
    body: JSON.stringify({
      ...
      folio: folio,
      fecha: fechaEmision,
      alumno: contenedor.innerText.match(/Alumno\s+(.+)/)?.[1] || 'Alumno individual',
      grupo: contenedor.innerText.match(/Grupo\s+(.+)/)?.[1] || 'Grupo individual',
      tipo:'INDIVIDUAL',
      url: urlValidacion
    })
  })
  .then(r => r.json())
  .then(data => {
    console.log('PDF registrado:', data);
  })
  .catch(error => {
  ...
  console.error('Error preparando registro PDF:', error);
}
